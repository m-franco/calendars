...
export const strings: Record<Lang, Dict> = {
    es: {
        ...
        subscribeLine: (subject) =>
            `Sincroniza tu calendario con los partidos de ${subject}.`,
        secondLine:
            "Los partidos se agregan al calendario de tu teléfono o computadora.",
    ...
    en: {
        homeTitle: "myFixtu.re",
        homeMetaDescription:
            "Sync your calendar with the World Cup matches and keep the fixture always at hand on your phone or computer.",
        homeHeadingA: "Sports",
        homeHeadingB: "calendars",
        homeInstructions:
            "Sync your calendar with the World Cup matches and keep the fixture always at hand on your phone or computer.",
        tournamentInstructions:
            "Sync your calendar with your team's, your whole group's or the entire cup's matches.",
        yearArchivePrefix: (year) => `${year} archive: `,
        subscribeLine: (subject) =>
            `Sync your calendar with ${subject}'s matches.`,
        secondLine: "The matches are added to your phone or computer calendar.",
    ...
    pt: {
        homeTitle: "myFixtu.re",
        homeMetaDescription:
            "Sincronize seu calendário com os jogos da Copa do Mundo e tenha a tabela sempre à mão no celular ou computador.",
        homeHeadingA: "Calendários",
        homeHeadingB: "esportivos",
        homeInstructions:
            "Sincronize seu calendário com os jogos da Copa do Mundo e tenha a tabela sempre à mão no celular ou computador.",
        tournamentInstructions:
            "Sincronize seu calendário com os jogos do seu time, de todo o grupo ou de toda a copa.",
        yearArchivePrefix: (year) => `Arquivo ${year}: `,
        subscribeLine: (subject) =>
            `Sincronize seu calendário com os jogos de ${subject}.`,
        secondLine:
            "Os jogos são adicionados ao calendário do seu telefone ou computador.",
...
> = {
    ...
    en: {
        ...
        intro: "If you'd like to help this project keep growing, you can make a contribution. Thank you!",
        button: "Support through PayPal",
    },
    pt: {
        nav: "Apoiar",
        title: "Apoie o projeto",
        intro: "Se você quer ajudar este projeto a continuar crescendo, pode fazer uma contribuição. Obrigado!",
...
export function subscribeCupText(tournament: string, lang: Lang): string {
    ...
    const wholeMap: Record<Lang, Record<string, string>> = {
        es: {
            mundial: "del Mundial",
            libertadores: "de la Copa Libertadores",
            sudamericana: "de la Copa Sudamericana",
        },
        en: {
            mundial: "World Cup",
            libertadores: "Libertadores Cup",
            sudamericana: "Sudamericana Cup",
        },
        pt: {
            mundial: "da Copa do Mundo",
            libertadores: "da Copa Libertadores",
            sudamericana: "da Copa Sul-Americana",
        },
    };
    const whole =
        wholeMap[lang][tournament] ?? tournamentTitle(tournament, lang);
    ...
    if (lang === "pt") {
        const grouping = isWC
            ? "todos os times de uma confederação"
            : "todos os times do seu país";
        const favorite = isWC ? "sua seleção" : "seu time";
        return `Sincronize seu calendário com os jogos ${whole} e tenha a tabela sempre à mão no celular ou computador. Você também pode sincronizar apenas o calendário da ${favorite}, de todo o grupo ou de ${grouping}.`;
    }
    const grouping = isWC ? "a confederation" : "your country";
    const favorite = isWC ? "your national team" : "your team";
    return `Sync your calendar with the ${whole} matches and keep the fixture always at hand on your phone or computer. You can also sync only ${favorite}'s calendar, your whole group's, or all the teams from ${grouping}.`;
}
...
export function subject(ctx: SubjectCtx): string {
    ...
    if (type === "team") {
        const teamName = localizedEntityName(
            type,
            path,
            ctx.esName,
            tournament,
    ...
    if (type === "tournament") {
        ...
        return { es: `la ${T}${yr}`, en: `the ${T}${yr}`, pt: `a ${T}${yr}` }[
            lang
        ];
    }
    // group
